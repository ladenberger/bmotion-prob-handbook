...
\begin{enumerate}
\item Clone the BMotion Studio for ProB standalone Github repository\footnote{\url{https://github.com/ladenberger/bmotion-prob-standalone}}.
\item Put your visualisation into the folder ``resources/workspace''.
\item Run the following command, where XXX is the path to the html template file in the resources folder (e.g. ``myvis/vis.html''):
\begin{lstlisting}[language=bash]
gradle -Pvisualisation="myvis/vis.html" buildAll
\end{lstlisting}
\item If you don't have gradle installed, you can use the gradlew script provided:
\begin{lstlisting}[language=bash]
./gradlew -Pvisualisation="myvis/vis.html" buildAll
\end{lstlisting}
\end{enumerate}
