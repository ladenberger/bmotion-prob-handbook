...
This should call the Event-B event \textit{push\_call\_button} with the corresponding parameter.
...
In Line 1 we register a new click function on the visual element with the id door.
Here we also use the jQuery selector syntax.
Line 2 calls our registered Groovy method \textit{openCloseDoor}.
In Line 3 to 5 we pass a callback function that can handle the returned value of the registered Groovy method \textit{openCloseDoor}.
